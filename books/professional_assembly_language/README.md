...
为了启动系统调用，需要使用INT指令。Linux系统调用位于中断0x80。执行INT指令时，所有操作转移到内核中的系统调用处理程序。系统调用完成之后，执行转移回INT指令之后的下一条指令。

在指令INT指令之前，需要准备好系统调用号和输入参数。`EAX`存放系统调用号，可以通过`asm/unistd.h`文件查询。通过寄存器传递参数，下面列出了参数位置和对应的寄存器，每个参数的含义可以通过`man`手册查询。
* EBX 第一个参数
* ECX 第二个参数
* EDX 第三个参数
* ESI 第四个参数
...
如果参数大于或等于六，则需要使用内存来传递参数。EBX寄存器保存指向输入参数的内存位置的指针，输入参数按照连续的顺序存储。系统调用使用这个指针访问内存位置以便读取参数。

以`write`系统调用为例，按照参数出现的顺序从左至右设置，即`fd`存储在`EBX`寄存器，`buf`存储在`ECX`寄存器，`count`存储在`EDX`寄存器。
...
Linux系统提供了strace程序，它在应用程序运行时跟踪其中使用的系统调用。

### strace程序

strace程序截取程序发出的系统调用并且显示它们以供查看。被跟踪的程序可以是从strace命令运行的，也可以是系统上已经运行的进程，这需要适当的权限。下面的程序显示了`strcomp`程序发出的系统调用，左侧显示系统调用名称，右侧显示系统调用的返回值。

```shell
$ strace ./strcomp
execve("./strcomp", ["./strcomp"], [/* 29 vars */]) = 0
brk(0)                                  = 0x9d62000
...
其中`-e`参数非常有用，可以过滤指定的系统调用。常见用法如下。更多用法请参考`man`手册。
...
`-p`参数附加到现在正在运行的程序，对调试后台程序非常有用。当然，这需要相应的权限。

## 系统调用和C库

C库提供了丰富的函数，可以在汇编语言程序中利用它们。

### C库

C库提供了很多有用的功能，其实现在`libc`中，必须把它链接到汇编程序中。C库函数的文档在`man`手册的第三部分，例如`man 3 exit`。主要关注函数功能、头文件、参数列表、返回值的含义。

C样式的函数使用堆栈传递参数，顺序和参数列表中的顺序相反。C调用`printf`是这样的：
```c
...
> 调用C库函数后，寄存器的值可能会发生改变。

### 跟踪C函数

部分C库函数需要通过底层系统调用完成它们的工作，所以也可以使用`strace`程序监视C函数的执行情况。注意，strace只会显示系统调用的名称和返回值，不会显示C库函数的名称和返回值。对于没有使用系统调用的C库函数（例如strcmp），无法看出其执行的情况。

### 系统调用和C库的比较

推荐使用系统调用是因为不依赖任何外部库，程序可以尽可能地小。

推荐使用C库是因为在操作系统级别是可移植的，不同程序之间可以共享内存，使用极少的汇编代码实现复杂的功能。

# 第十三章 使用内联汇编

介绍如何把汇编语言函数直接放到C和C++语言程序内，这种技术称为内联汇编（inline assembly）。

## 什么是内联汇编

在有些情况下，不希望使用编译器生成的汇编代码，或使用编译器不可能生成的汇编语言指令，如果想直接控制汇编代码去实现功能，有三种选择：
1. 汇编库。从头开始编写汇编语言代码，然后从C程序调用。
2. 手工优化。使用-S选项创建C代码的汇编语言版本，必要的情况下手工修改汇编语言代码。
3. 内联汇编。在原始的C代码内编写汇编语言代码。

## 基本的内联汇编

### asm格式

GNU的C编译器使用asm关键字指出使用汇编语言编写的源代码段落。基本格式如下：
...
括号中的汇编代码必须使用特定的格式：
* 指令必须括在引号里。
* 如果包含多条指令，必须用换行符分隔每一行。通常还包含制表符缩进汇编代码。
...
> 调试内联汇编最好的方法是使用`-S`查看汇编代码。

### 使用全局C变量

可以使用全局变量将数据传递进内联汇编，或传递出。
...
从汇编的角度看，全局变量a的定义如下。所以内联汇编`movl a, %eax`表示加载`a`的地址到EAX寄存器。
...
编译器可能会优化汇编代码。对内联汇编来说可能是一件不好的事情，可以使用`volatile`关键字阻止优化。
...
`ASIN C`把asm关键字用于其他用途，不作为内联汇编语句。如果使用`ASIN C`约定编写代码，可以使用`__asm__`和`__volatile__`。例如：
...
基本的asm格式太过于简单，限制较多，所以就有了扩展asm来解决基本asm的问题。
...
```c
asm("assembly code" : output locations : input operands : change registers)
```

这种格式由4个部分构成，使用冒号分隔：
* 汇编代码：使用和基本asm格式相同的语法和内联汇编代码
* 输出位置：包含内联汇编代码的输出值的寄存器和内存位置的列表
* 输入操作数：包含内联汇编代码的输入值的寄存器和内存位置的列表
* 改动的寄存器：内联代码改变的任何其他寄存器的列表（不含输出位置和输入操作数）

如果某一部分没有，可以省略，但是必须保留冒号分隔。
...
`variable`是程序中声明的C变量，在扩展asm格式中，全局和局部变量都可以使用。`constraint`（约束）定义把变量存放在哪里（对于输入值）或者从哪里传送变量（对数输出值）。使用它定义把变量存放在寄存器中还是内存位置中。`constraint`是单一字符的代码，约束代码如下表。
...
输出值还包含一个约束修饰符，指示编译器如何处理输出值。可用的输出修饰符如下表所示：
...
如果输入值和输出变量被赋值给寄存器，那么在内联汇编代码中几乎可以像平常一样使用寄存器。但是在扩展asm格式中，寄存器需要使用两个百分号。
...
int main()
{
    ...
    asm("imull %%edx, %%ecx \n\t"
        "movl %%ecx, %%eax"
        : "=a"(result)                  // result 存储在eax寄存器，只能写入
        : "d"(data1), "c"(data2));      // data1存储在edx，data2存储在ecx
...
```asm
        movl    $10, -16(%ebp)     # 设置局部变量 data1 的值
        movl    $20, -12(%ebp)     # 设置局部变量 data2 的值
        movl    -16(%ebp), %edx    # 将局部变量赋值给edx，"d"(data1)
        movl    -12(%ebp), %ecx    # 将局部变量赋值给ecx，"c"(data2)
#APP
        imull %edx, %ecx
        movl %ecx, %eax
#NO_APP
        movl    %eax, -8(%ebp)     # 结果在eax，并传送到局部变量 result，"=a"(result)
```

不一定总要在内联汇编段中指定输出值，因为一些指令已经假定输入值包含输出值，例如movs指令。由于没有指定输出值，所以需要加上volatile，避免被优化。
...
在输入输出值不多的情况下，可以手工指定输入值和输出值的寄存器。输入输出值很多的情况下，就很麻烦了。asm扩展格式提供了占位符，让编译器自动的选择合适的寄存器或内存位置。

占位符是前面加上百分号符号的数字。按照内联汇编代码中列出的每个输入值和输出值在列表中的位置，每个值被赋予一个从零开始的数字。然后就可以在汇编代码中使用占位符表示值。

> 必须把输入值和输出值声明为内联代码中汇编指令需要的正确的存储元素（寄存器或内存）
...
int main()
{
    ...
    asm("imull %1, %2 \n\t"
        "movl %2, %0"
        : "=r"(result)                  // %0 result, 约束字符是r，所有可用的通用寄存器
...
```asm
        movl    $10, -16(%ebp)
        movl    $20, -12(%ebp)
        movl    -16(%ebp), %edx    # data1
        movl    -12(%ebp), %eax    # data2
#APP
        imull %edx, %eax
        movl %eax, %eax            # 非常有趣，eax传递给eax
#NO_APP
        movl    %eax, -8(%ebp)
```

> 调试内联汇编时，GDB认为asm段是单一语句。可以使用`stepi`命令进入asm段进行单步执行，单独地执行每条指令。

### 引用占位符

如果内联汇编代码中的输入值和输出值共享程序中相同的C变量，可以指定使用占位符作为约束值。便于减少代码中需要的寄存器数量。例如可以编写下面的代码，data2同时作为输入和输出。
...
当输入输出值较多时，数字型的占位符很容易混乱。为了便于阅读，GNU编译器允许声明替换的名称作为占位符。替换的名称在声明输入值和输出值的段中定义。格式如下：
```
[name]"constraint"(variable)
```

使用`%[name]`引用占位符。例如下面的代码：
...
内联汇编代码也可以包含定义其中位置的标签。可以实现一般的汇编条件分支和无条件分支，跳转到定义的标签。
...
在内联汇编使用标签有以下限制：
1. 只能跳转到相同asm段内的标签。
2. 标签会被编码到最终汇编后的代码，所以不能重名。
3. 标签名不能使用C关键字或C变量名（函数名称、变量名）。

可以使用局部标签解决以上限制。使用数字型标签，并且在条件分支加上方向标签，第一个遇到的标签会被采用。
...
内联汇编代码必须使用扩展asm格式，以便定义正确的输入值和输出值。因为程序中可能多次使用宏函数，条件分支还应该使用数字型的标签。asm语句必须在一对大括号中。
...
> 这个宏使用参数来返回值，调用的时候不太优雅。如何让结果作为宏的返回值？临时变量记录返回值，使用逗号表达式返回值。

# 第十四章 调用汇编库

本章讨论C和C++程序如何直接调用汇编语言函数，把输入值传递给函数，并且从函数获得输出值。

## 创建汇编函数

如果希望汇编语言函数和C/C++程序一起工作，就必须遵守C样式的函数格式。如下图所示，更详细的内容参考[第十一章 使用函数](#第十一章-使用函数)。

![](pic/函数堆栈.png)

被调用的汇编函数必须保留EBX、EDI、ESI和ESP寄存器，如果更改了，必须恢复。下表列出了常用寄存器在汇编函数中的作用。
...
func:
    pushl %ebp
    movl %esp, %ebp
    subl $12, %esp     # 增加栈空间，为局部变量预留
    pushl %edi         # 保存寄存器的值
    pushl %esi         # 如果某些寄存器函数没有使用，可以不保存
    pushl %ebx
...
在C++程序中使用汇编语言函数的规则几乎和在C程序中使用它们的规则相同，只是函数声明要放在`extern{}`中。因为C++有自己的一套命名和调用约定，与C不兼容，所以要告诉编译器这些函数使用C样式的调用约定，这通过`extern{}`来完成。
...
本节介绍库如何工作，以及如何创建可以用于编译C程序的汇编函数库。

### 什么是静态库

把编译好的目标文件组合在单一存档文件中，这个存档文件就称为静态库。编译时只需指定一个静态库，编译器会自动选择出需要的目标文件。静态库中包含的目标代码被链接到了主程序中，所以运行时不需要这些静态库了。

### ar命令

Linux环境中使用ar命令创建静态库文件。
...
| 修饰符 | 描述 |
| - | - |
| a | 把新地文件添加到存档文件中现有的文件之后 |
| b | 把新地文件添加到存档文件中现有的文件之前 |
| c | 创建新的存档文件 |
| f | 截短存档文件中的名称 |
...
ar命令的格式如下，archive是静态库的名称，files是库文件中包含的目标文件的清单，用空格分隔。
```shell
ar [-]{dmpqrtx}[abcfilNoPsSuvV] [merbername] [count] archive files...
```

### 创建静态库文件

在创建静态库文件之前，必须先创建目标文件，可以使用as命令的`-o`选项。Linux使用如下的格式命名静态库文件，`lib`是固定前缀，`x`是静态库的名字，后缀`a`表明是静态库文件。
```shell
libx.a
```
...
创建新的静态库或添加新文件
```shell
ar r libxxx.a obj1.o obj2.o...
```

查看静态库中的目标文件
```shell
ar t libxxx.a
ar tv libxxx.a # 查看详细信息
```

创建静态库的索引（每个目标文件包含哪些函数），加速编译
```shell
ranlib libxxx.a  # 无任何输出，索引存储在静态库中
```

查看静态库的索引
```shell
nm -s libxxx.a   # 每个函数的名称及其所在的目标文件
                 # 最后是每个目标文件的符号的详细清单
```

### 编译静态库

将静态库添加到命令行参数即可，与目标文件的使用方法一样。

## 使用共享库

略。请参考《程序员的自我修养——链接装载和库》。

## 调试汇编函数

使用GDB调试的文件需要使用`-g`参数编译。关于GDB的更多使用方法，请参考[使用GDB](#使用gdb)或[GDB常用指令](https://pkemb.com/2021/05/gdb-command/)。
...
本章介绍如何使用汇编语言从文件种读取数据、写入数据到文件中。使用两种方法访问文件，一种是标准的C函数，一种是Linux系统调用。
